...
const transportPreference=document.getElementsByName('transport-method');
const musicPreference=document.getElementById('music-preference');
 //default for better experience
 musicResult.textContent=musicPreference.value;
/*
 * CHECKBOX LISTENERS
 */
...
for(let i=0;i< transportPreference.length;i++){
  transportPreference[i].addEventListener('change', ($event)=>{
    transportResult.textContent= $event.target.value;
  });
}

musicPreference.addEventListener('change',($event)=>{
